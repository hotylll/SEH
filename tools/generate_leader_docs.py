...
TEAM = [
    ("罗元恒", "罗元恒", "项目总负责人，核心文档编写，架构设计，全程管理"),
...
MODULES = [
    ...
    ("配置管理与审计扩展模块", "负责数据源配置、阈值配置、审计日志，并预留账号权限扩展。"),
]
...
REQUIREMENTS = [
    ...
    ("REQ-017", "系统后续扩展阶段应提供管理员、分析员、普通用户三类角色及权限控制", "后续扩展", "TC-017-FUTURE"),
    ("REQ-018", "系统应记录配置修改、任务启动、报表导出等关键操作日志，用户登录审计作为后续扩展", "审计", "TC-018"),
...
API_ENDPOINTS = [
    ...
    ("POST", "/api/v1/tasks/collect", "启动采集任务", "{source_id}", "{id,status,success_count,failed_count}", "演示用户"),
    ("GET", "/api/v1/tasks/{task_id}", "查询任务状态", "task_id", "{status,success_count,failed_count,error_message}", "管理员/分析员"),
    ("GET", "/api/v1/items", "检索信息明细", "keyword,source_id,limit", "{items,total}", "演示用户"),
    ("GET", "/api/v1/items/{item_id}", "查看信息详情", "item_id", "{id,raw_id,title,content,keywords,quality_score}", "演示用户"),
    ("GET", "/api/v1/trends", "查询趋势榜单", "source_id,limit", "{items,total}", "演示用户"),
    ("GET", "/api/v1/trends/{topic}", "查看主题趋势", "topic", "{topic,series,related_items}", "演示用户"),
    ("GET", "/api/v1/reports", "查询报表记录", "无", "{items,total}", "演示用户"),
    ("POST", "/api/v1/reports", "生成分析报表", "{report_type,format,generated_by}", "{id,report_name,download_url}", "演示用户"),
    ("GET", "/api/v1/reports/{report_id}/download", "下载报表文件", "report_id", "xlsx/pdf 文件流", "演示用户"),
...
def add_common_front_matter(doc: Document, spec: DocSpec) -> None:
    ...
    add_table(
        ...
        [
            ...
            ["V1.2", REVISION_DATE, f"最终格式统一与交付前核对", AUTHOR],
        ],
    )
...
def recurring_paragraphs(focus: str, count: int) -> list[str]:
    base = [
        ...
        f"{focus}这块跟分工是绑定的：崔昊晨管采集，罗昆昊管清洗和存储，兰玉杰管趋势分析算法，王永成管前端展示和用户文档，郭子凌管测试和进度月报，罗元恒我管整体架构、核心文档、评审把关和最终归档。各管各的，接口对齐就行。",
    ]
            ...
            Section(
                ...
                tables=[
                    (
                        ["阶段", "时间", "主要任务", "责任人", "交付物"],
                        [
                            ["可行性与计划", "2026-06-13 至 2026-06-14", "选题、竞品分析、总体计划", "罗元恒、兰玉杰", "项目开发计划、可行性研究报告"],
                            ["需求分析", "2026-06-14 至 2026-06-16", "需求调研、用例建模、数据要求", "罗元恒、崔昊晨", "软件需求说明书、数据要求说明书"],
                            ["系统设计", "2026-06-16 至 2026-06-18", "架构、概要设计、数据库设计、详细设计", "罗元恒、罗昆昊", "概要设计、数据库设计、详细设计"],
                            ["编码开发", "2026-06-19 至 2026-06-21", "各模块编码、联调和模块卷宗", "全体成员", "代码、模块开发卷宗、进度月报"],
                            ["测试验收", "2026-06-22 至 2026-06-23", "测试计划执行、缺陷修复、手册编写", "郭子凌、王永成", "测试计划、测试分析、用户手册"],
                            ["总结答辩", "2026-06-23 至 2026-06-24", "部署验收、总结、演示检查和归档", "罗元恒", "开发总结报告、演示检查材料"],
                        ],
                    )
                ],
            ),
...
def requirements_spec() -> DocSpec:
    functional_sections = [
        ...
        Section("配置与后续权限扩展需求", recurring_paragraphs("配置与后续权限扩展需求", 2)),
    ]
    return DocSpec(
        "软件需求说明书.docx",
        "软件需求说明书",
        ...
        [
            ...
            Section(
                "用户与业务场景",
                [
                    "当前演示版本以罗元恒现场操作为主，重点展示数据源配置、采集、检索、趋势详情和报表下载。管理员、分析员和普通用户三类账号作为后续权限扩展设计，不纳入本次课堂演示必须项。",
                ...
                tables=[
                    (
                        ["用户角色", "主要目标", "关键操作"],
                        [
                            ["罗元恒/演示操作者", "完成课堂演示和数据配置", "配置数据源、启动采集、查看趋势、下载报表"],
                            ["信息分析员", "完成热点分析和趋势研判", "检索信息、筛选主题、查看趋势图、导出报告"],
                            ["后续普通用户", "浏览系统整理后的信息", "查看热点榜、查看信息详情、按关键词搜索"],
                        ],
                    )
                ],
            ),
    ...
    return DocSpec(
        ...
        [
                ...
                tables=[
                    (
                        ...
                        [
                            ...
                            ["接口层", "REST API、参数校验、统一错误响应", "负责前后端通信和统一访问入口"],
                            ["业务层", "采集、清洗、分析、配置、报表", "承载系统主要业务逻辑"],
            ...
            Section(
                "接口设计",
                [
                    "接口采用 REST 风格，统一前缀为 /api/v1。当前演示版本重点实现参数校验、400/404 错误区分和 CORS 支持，登录令牌和权限拦截作为后续扩展。",
            ...
            Section("安全与后续权限设计", recurring_paragraphs("安全与后续权限设计", 3)),
...
def database_spec() -> DocSpec:
    entity_rows = [
        ["users", "用户账号预留表", "保存后续登录用户、角色、状态和创建时间"],
        ...
        ["audit_logs", "审计日志表", "保存采集、配置、报表等关键操作、目标对象和发生时间"],
    ]
    return DocSpec(
        "数据库设计说明书.docx",
        "数据库设计说明书",
        ...
        [
            ...
            Section(
                "概念结构设计",
                [
                    "核心实体有这么几个：数据源、采集任务、原始信息、清洗信息、趋势结果、报表、审计日志，以及后续权限扩展预留的用户表。基本就是数据从进来到出去的整条链路上要存的东西。",
            ...
            Section(
                "逻辑结构设计",
                [
                    "我们把业务数据和管理数据分开。data_sources → collect_tasks → raw_items → clean_items 是数据生产主线，topic_trends 和 reports 是分析输出线，users 和 audit_logs 是管理审计和后续权限扩展线。三条线互不干扰。",
            ...
            Section(
                ...
                [
                    ...
                    "审计日志采用追加写入方式，不允许普通业务流程修改历史日志。对报表文件路径、采集任务和导出时间进行记录，便于答辩时说明系统具备基本工程治理能力。",
                ],
            ),
...
def tech_selection_spec() -> DocSpec:
    rows = [
        ["后端框架", "Python 标准库 HTTP 服务", "FastAPI、Flask", "依赖少、部署简单、适合课程演示和本地运行"],
        ["数据库", "SQLite", "PostgreSQL、MySQL", "关系模型清晰，SQLite 便于演示，后续可迁移到 PostgreSQL"],
        ["全文检索", "内置LIKE/后续扩展Elasticsearch", "Meilisearch", "课程阶段先保证可运行，后续可替换为专业检索引擎"],
        ["前端", "原生 HTML/CSS/JavaScript", "Vue或React", "无需构建工具，直接打开即可演示，便于课堂环境复现"],
...
def trace_spec() -> DocSpec:
    ...
    for req_id, desc, module, test in REQUIREMENTS:
        status = "后续扩展" if "FUTURE" in test else "已覆盖"
        rows.append([req_id, desc, f"概要设计-{module}模块", f"数据库/接口-{module}对象", test, status])
    return DocSpec(
        "需求追溯矩阵.docx",
        "需求追溯矩阵",
        "项目管理补充文档",
...
def meeting_spec() -> DocSpec:
    rows = [
        ...
        ["第5次", "2026-06-24", "提交前最终核对", "检查全套文档格式一致性、版本记录完整性、图表编号规范性，确认交付物齐全"],
    ]
    return DocSpec(
        "会议纪要与评审记录.docx",
        "会议纪要与评审记录",
    ...
    appendices: dict[str, tuple[str, list[str], list[tuple[list[str], list[list[str]]]]]] = {
        "项目开发计划.docx": (
            ...
            [
                ...
                "罗元恒每周汇总一次成员进展，重点检查是否存在接口不一致、数据格式不一致、文档模板不一致和测试覆盖不足等问题。发现偏差后先明确责任人和完成日期，再决定是否调整后续计划。",
                "资源安排采用“核心流程优先”的策略。若时间不足，优先保证采集到清洗、清洗到分析、分析到展示的闭环可以演示；报表样式、复杂检索和高级算法作为增强项排在主流程之后。",
                "凡涉及需求范围、数据库结构、接口字段和演示路径的变更，均由罗元恒记录变更原因、影响范围和确认人，避免成员各自修改导致最终材料不一致。",
            ],
            [
                (
                    ["里程碑", "验收条件", "不通过时处理方式"],
                    [
                        ["需求基线", "需求编号完整，用户角色、功能需求、非功能需求和验收标准均已确认", "由罗元恒组织需求复审，冻结前不得进入概要设计"],
        ...
        "软件需求说明书.docx": (
            ...
            [
                ...
                "需求变更必须同步更新需求编号、设计章节、数据库字段、接口定义和测试用例。若只修改某一份文档，会导致答辩时不同材料口径不一致，因此罗元恒负责维护需求追溯矩阵作为唯一核对表。",
                "边界条件也纳入验收范围，包括空数据源、重复信息、采集失败、非法参数、超出范围查询和报表生成失败等场景。系统即使不能完成业务操作，也必须给出明确错误提示和日志记录。",
            ...
            [
                (
                    ["优先级", "需求范围", "验收方式"],
                    [
                        ["P0 必须实现", "采集、清洗、存储、趋势分析、基础展示、详情查看、报表下载", "通过主流程演示和接口测试验收"],
                        ["P1 应当实现", "报表导出、审计日志、任务重试、组合检索", "通过功能测试和数据检查验收"],
                        ["P2 后续扩展", "用户登录、角色权限、真实爬虫接入", "作为后续版本规划说明"],
        ...
        "概要设计说明书.docx": (
            ...
            [
                ...
                "错误响应按场景划分：400 表示参数或请求体错误，404 表示资源不存在，500 表示服务端异常。前端根据错误信息展示可理解提示，后端保留必要日志便于排查。",
            ...
            [
                (
                    ...
                    [
                        ...
                        ["审计记录", "执行采集或报表生成后查询 audit_logs", "存在动作、目标和时间记录"],
                    ],
                )
            ],
        ),
        "项目开发总结报告.docx": (
            "附录：成员贡献记录口径",
            [
                "总结报告中的成员贡献以可交付物为依据，包括文档、代码、测试用例、评审记录、缺陷修复和演示材料。罗元恒贡献重点体现在整体架构、核心文档、统一模板、进度控制和最终归档。",
                ...
                "问题复盘采用“现象、原因、处理、预防”四段式记录。例如接口字段不一致时，现象是前端表格缺字段，原因是需求变更未同步，处理是统一接口表，预防措施是由罗元恒维护接口变更记录。",
                "罗元恒在总结中应突出组织贡献：统一选题方向、拆分模块边界、制定文档模板、维护需求追溯、组织阶段评审、推动最终归档。这些内容能体现软件工程管理能力，而不仅是单份文档编写工作。",
                "最终归档按“文档、代码、测试、演示、管理记录”五类整理，文件名保持清晰可读，并在提交前由罗元恒逐项核对，避免出现缺页、错版或成员材料口径不一致。",
                "提交前保留最终版与可编辑源文件。",
                "归档清单须编号。",
                "最终版本锁定。",
                "完成。",
        ...
        "技术选型论证报告.docx": (
            ...
            [
                ...
                "当前版本采用 Python 标准库 HTTP 服务、SQLite、原生 HTML/CSS/JavaScript 和 Docker Compose，优先保证课堂演示稳定；FastAPI、PostgreSQL、Vue/React 可作为后续扩展方案。",
            ],
            [
                (
                    ["方案", "学习成本", "实现效率", "演示稳定性", "扩展能力", "综合结论"],
                    [
                        ["Python 标准库 HTTP 服务", "5", "4", "5", "3", "当前版本采用，适合本地演示"],
                        ["FastAPI", "4", "5", "5", "4", "后续接口文档化和扩展时可迁移"],
