                ...
                tables=[
                    (
                        ...
                        [
                            ...
                            ["总结答辩", "第9周", "部署验收、总结、PPT和归档", "组长", "开发总结报告、答辩材料"],
                        ],
                    )
                ],
            ),
...
def ppt_spec() -> DocSpec:
    rows = [
        ["1", "项目背景", "说明多源信息分散、热点识别困难、系统建设目标"],
        ["2", "需求分析", "展示用户角色、核心需求、需求追溯关系"],
        ["3", "总体架构", "展示采集、清洗、存储、分析、展示的处理链路"],
        ["4", "数据库设计", "展示核心实体、表结构和数据流转"],
        ["5", "系统演示", "展示采集任务、趋势分析、图表和报表导出"],
        ["6", "测试与总结", "展示测试覆盖、问题处理、成员贡献和改进方向"],
    ]
    return DocSpec(
        "答辩PPT内容大纲.docx",
        "答辩PPT内容大纲",
        "项目管理补充文档",
        "PPT-001",
        "本文档规划结题答辩PPT的页面结构、讲述重点、演示顺序和分工，用于后续制作正式演示材料。",
        1500,
        [
            Section("答辩目标", recurring_paragraphs("答辩PPT", 2)),
            Section("页面结构", ["PPT控制在8至12页，优先展示工程过程和系统闭环。"], tables=[(["页码", "页面主题", "讲述重点"], rows)]),
...
def add_review_appendix(doc: Document, spec: DocSpec) -> None:
            ...
            [
                (
                    ...
                    [
                        ...
                        ["交付基线", "文档、代码、测试报告、PPT和演示材料齐全", "按缺失清单补齐并重新验收"],
                    ],
                )
            ],
        ),
    ...
    default = (
        ...
        [
            ...
            "所有补充材料必须能说明其用途：追溯矩阵用于验收覆盖，会议纪要用于过程管理，PPT大纲用于答辩组织，图表文档用于概要设计和展示复用。",
        ],
        [
            (
                ["检查项", "检查要求"],
...
def build_document(spec: DocSpec) -> tuple[Path, int]:
    ...
    if spec.filename == "图1-系统架构与数据流.docx":
        ...
        add_paragraph(doc, "本节嵌入根据系统设计直接绘制的正式图表，可在答辩PPT、概要设计说明书和项目开发计划中复用。")
